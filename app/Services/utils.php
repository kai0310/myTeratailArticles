...
class utils{
    ...
    public function searchWord(Array $replies,Array $searchWords) {
        $filter = function($replies) use ($searchWords){
            return $this->wordMatching($replies['body'], $searchWords);
        };
        return array_filter($replies, $filter);
    }
    /**
     * 配列内から全角・半角・大文字・小文字を無視して一致する内容を返却する。
     * @param string $searchObject 検索対象
     * @param Array $searchWords 探したい文字配列
     * @return Array
     */
    private function wordMatching(string $searchObject,Array $searchWords){
        $stringMatching = function($searchValue) use ($searchObject){
            if(mb_stripos(mb_convert_kana($searchObject),mb_convert_kana($searchValue))!==false){
                return true;
            }            
            return false;
        };
